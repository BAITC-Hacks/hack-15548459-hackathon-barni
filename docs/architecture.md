...
flowchart LR
    ...
    G --> S
    G --> W
    G -. "интеграция ожидает доработки" .-> U["UI: поиск узла и граф сети — пока нет"]
    C -. "инструменты графа пока не подключены" .-> A["AI-ассистент по данным графа — пока нет"]
```
...
- **UI и AI** — общее приложение харнеса существует, но экран графа/поиска по GID и инструмент `graph_tools` пока отсутствуют. Соединения на схеме пунктиром обозначают запланированную интеграцию, а не готовую функцию.
