...
- Показывает в Streamlit сеть, список приоритетов, кластеры и карточки узлов. По умолчанию без поиска и выбранного кластера граф показывает до 50 узлов с наивысшим приоритетом; любое представление сети ограничено 300 отображаемыми узлами, а CSV-файлы содержат полный результат.
...
| Интерфейс | Streamlit, PyVis/vis-network | Граф, поиск GID, карточки, топ и кластеры |
...
1. Запускает расчёт и открывает отчёт с общими результатами.
2. Смотрит топ приоритетов, затем открывает карточки выбранных `gid`.
3. Сверяет роль и `rule` с `evidence`, суммами, числом плательщиков и получателей, датами и паттернами.
4. Исследует связи узла или кластер в интерфейсе; при настроенном ключе задаёт вопросы AI-ассистенту, который использует `node_card`, `neighbors`, `path`, `top_by_role` и `common_receivers`.
...
Основные каталоги: `data/` — входные Parquet-файлы; `pipeline/` — расчёты; `outputs/` — результаты; `ui/` — интерфейс и просмотрщик; `harness/` — AI-агент и его инструменты; `docs/` — критерии, схема и сценарий; `tests/` — проверки выгрузок и компонентов.
...
| Топ приоритетов и экран сети | `outputs/top_nodes.csv` и вкладки Streamlit; экран ограничивает визуальную сеть 300 узлами |
