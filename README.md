...
Основные каталоги: `data/` — входные Parquet-файлы; `pipeline/` — расчёты; `outputs/` — результаты; `ui/app.py` — главное приложение, `ui/aml/` — его модули, `ui/viewer.py` — отдельный просмотрщик; `harness/` — AI-агент и его инструменты; `docs/` — критерии, схема и шпаргалка жюри; `tests/` — проверки выгрузок и компонентов.
